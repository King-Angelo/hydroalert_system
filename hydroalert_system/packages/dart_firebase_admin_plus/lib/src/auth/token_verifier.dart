...
class DecodedIdToken {
  ...
  factory DecodedIdToken.fromMap(Map<String, Object?> map) {
    // Firebase puts sign_in_provider, identities, and MFA fields under the
    // `firebase` claim — not at the JWT root. Upstream 0.5.0 read
    // `map['sign_in_provider']`, which is null for Google / OIDC sign-in and
    // caused "Null check operator used on a null value" during verifyIdToken.
    final firebaseClaim = map['firebase'];
    if (firebaseClaim is! Map) {
      throw FirebaseAuthAdminException(
        AuthClientErrorCode.invalidArgument,
        'Firebase ID token missing firebase claim.',
      );
    }
    final firebaseMap = Map<String, Object?>.from(firebaseClaim);
    final identitiesRaw = firebaseMap['identities'];
    if (identitiesRaw is! Map) {
      throw FirebaseAuthAdminException(
        AuthClientErrorCode.invalidArgument,
        'Firebase ID token missing firebase.identities.',
      );
    }
    final sub = map['sub']! as String;
    final uid = (map['uid'] ?? map['user_id'] ?? sub) as String;
    return DecodedIdToken(
      ...
      firebase: TokenProvider(
        identities: Map<String, Object?>.from(identitiesRaw),
